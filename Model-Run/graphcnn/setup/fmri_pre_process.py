...
class PreProcess(object):
    def __init__(self, atlas='', node_number=90, window_size=100, step=1, proportion=0.2,
        ...
        self.gcn_edge_weight_matrix_binary = []

    def compute_graph_cnn_input(self):
        self.compute_input_node_signal()
        return np.array(self.gcn_node_feature), np.array(self.gcn_edge_weight_matrix_binary), np.reshape(
            self.gcn_label, [-1])

    def compute_input_node_signal(self):
        self.compute_edge_weight()
        self.compute_edge_weight_binary()
        self.gcn_edge_weight_matrix_binary = np.expand_dims(self.edge_weight_matrix_binary, axis=3)
