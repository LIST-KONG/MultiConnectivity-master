...
class GAT(nn.Module):
    def __init__(self, args):
        ...
        self.sc_conv1 = GATConv(self.sc_features, self.hidden_dim)
        self.sc_convs = torch.nn.ModuleList()
        for i in range(self.num_layers - 1):
            self.sc_convs.append(GATConv(self.hidden_dim, self.hidden_dim))

        self.fc_conv1 = GATConv(self.fc_features, self.hidden_dim)
        self.fc_convs = torch.nn.ModuleList()
        for i in range(self.num_layers - 1):
            self.fc_convs.append(GATConv(self.hidden_dim, self.hidden_dim))
